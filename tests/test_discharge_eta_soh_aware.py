...
Qt-coupled (BaseSequenceMixin), so verified via a headless smoke-test-style
unittest rather than mocking the whole widget tree — this mirrors how
_project_tail_eta/_estimate_charge_s are tested elsewhere.
"""
import unittest
...
class _Stub:
    _estimate_discharge_s = BaseSequenceMixin._estimate_discharge_s
...
class TestEstimateDischargeS(unittest.TestCase):
    ...
    def test_exception_safe_returns_zero(self):
        class _Broken:
            controller = None
        est = BaseSequenceMixin._estimate_discharge_s(_Broken(), 1.0)
