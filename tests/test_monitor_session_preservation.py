"""Regression test: start_monitor() must NOT clobber a session a sequence
already opened.
...
class TestStartMonitorPreservesOpenSession(unittest.TestCase):
    ...
    def test_existing_labelled_session_survives_start_monitor(self):
        """The exact PREPARE -> start_charge -> start_monitor flow."""
        ...
        self.assertEqual(self.data.current_path, labelled,
                         "start_monitor() replaced the sequence's open session")
    ...
    def test_fresh_start_monitor_still_creates_a_session(self):
        """Manual Start Monitor with nothing recording keeps its old behaviour."""
