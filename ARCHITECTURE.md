...
- **UI:** **PySide6 + PyQtGraph** (`main_qt.py` — GUI ใหม่ 5-panel, ใช้ logic เดิมทั้งหมด;
  cross-thread update ผ่าน Qt signal/slot + `QtRootShim.after()`). Tkinter (`main.py`) ยังรันได้เป็น fallback
...
python main_qt.py                      # ★ GUI ใหม่ (PySide6 5-panel) — แนะนำ
python main.py                         # GUI เดิม (Tkinter) — fallback
